...
A detection confidence threshold of 0.25 is applied, corresponding to the Ultralytics YOLOv8 default inference threshold [Ultralytics YOLOv8 https://github.com/ultralytics/ultralytics]. This value prioritizes detection recall to prevent undercounting the surrounding traffic population, which would directly distort the Stationary Ratio $R$. The detection module provides standardized bounding box inputs to the subsequent tracking stage.
...
In monocular surveillance geometry, the bottom-center point approximates the vehicle's physical contact point with the pavement, satisfying the coplanar road-surface constraint ($Z = 0$) required for planar homography projection [Application of 2D Homography for High Resolution Traffic Data Collection using CCTV Cameras https://arxiv.org/abs/2401.07220]. Utilizing the vertical centroid of the bounding box would introduce perspective height distortion that scales unfavorably with camera depression angle. As established in geometric traffic perception literature, partial vertical occlusion from guardrails, roadside barriers, or intervening tall vehicles truncates the lower boundary of the bounding box, elevating the observed bottom-center coordinate above the true road surface ($Z > 0$) [Illegal Parking Detection Based on Multi-Task Driving Perception https://doi.org/10.1109/iv55156.2024.10588798], [Real-Time Occlusion-Aware Vehicle Detection and Behavior Analysis Framework for Intelligent Traffic Enforcement https://doi.org/10.1109/icces67310.2025.11336884]. Projecting an elevated point through a planar homography matrix $\mathbf{H}$ produces a well-documented geometric distortion that projects the estimated point further along the camera ray [Application of 2D Homography for High Resolution Traffic Data Collection using CCTV Cameras https://arxiv.org/abs/2401.07220]; therefore, the displacement computation operates under the standard computer vision assumption of unobstructed road-contact visibility.
...
The stationary noise ceiling $\varepsilon_s(Y_t)$ is calibrated empirically using known-stationary vehicles recorded for a minimum of 60 seconds across three depth zones (near-field, mid-field, and far-field). Because perspective projection causes constant pixel-level detection jitter to represent larger metric displacements at greater distances from the camera [Application of 2D Homography for High Resolution Traffic Data Collection using CCTV Cameras https://arxiv.org/abs/2401.07220], [A flexible new technique for camera calibration https://doi.org/10.1109/34.888718], applying a uniform global noise ceiling would misclassify creeping near-field vehicles as stopped. To resolve this, $\varepsilon_s(Y_t)$ is formulated as a depth-dependent linear interpolation scaled by the metric $Y$-coordinate across the calibrated positions, incorporating a 1.5× safety multiplier above the measured jitter to accommodate camera vibration.

To trigger an evaluation event, a per-vehicle dwell timer increments for each vehicle that maintains a stationary classification ($d(v, t) < \varepsilon_s$) within the restricted zone, resetting to zero whenever motion is detected. When the dwell timer reaches the temporal threshold $T = 60$ seconds, a trigger event is generated for that Track ID. The 60-second duration is adopted directly from established fixed-camera parking surveillance literature [Enhanced Detection of Illegally Parked Vehicles Using YOLO and Good Feature to Track Methods https://doi.org/10.1109/apsipaasc63619.2025.10849019], [Parking Violation Detection Using Computer Vision for Urban Traffic Management https://doi.org/10.1109/iciss63372.2025.11076537], [Outdoor Illegal Parking Detection System Using Convolutional Neural Network on Raspberry Pi https://doi.org/10.14419/ijet.v7i3.7.16197], [Illegal Parking Detection Using Gaussian Mixture Model and Kalman Filter https://doi.org/10.1109/aiccsa.2017.212], where legitimate operational pauses (such as pedestrian yielding or momentary queueing) consistently resolve well below this temporal threshold under normal urban traffic flow.
...
The theoretical formulation of $R(t)$ is rooted in Lighthill and Whitham's Kinematic Wave Theory [On kinematic waves. II. A theory of traffic flow on long crowded roads https://doi.org/10.1098/rspa.1955.0089] and spatiotemporal traffic analysis [Detection of Parked Vehicles Using Spatiotemporal Maps https://doi.org/10.1109/tits.2011.2156791], [Real-Time Illegal Parking Detection Algorithm in Urban Environments https://doi.org/10.1109/TITS.2022.3180225]. In urban traffic flow, legitimate traffic stoppages represent collective, causally coupled deceleration states that propagate across neighboring vehicles, causing $R(t) \to 1.0$. Conversely, an illegal parking violation represents an isolated spatial anomaly where the candidate remains stationary while surrounding traffic maintains independent movement, yielding $R(t) \to 0.0$.

To provide statistical weighting for the Stationary Ratio, the system records the Surrounding Vehicle Count $C = |\mathcal{S}(t_{\text{trig}})|$. Grounded in traffic density estimation literature [A New Curb Lane Monitoring and Illegal Parking Impact Estimation Approach Based on Queueing Theory and Computer Vision for Cameras with Low Resolution and Low Frame Rate https://doi.org/10.1016/j.tra.2022.05.024], [Detection of Illegal Parking Events Using Spatial-Temporal Features https://doi.org/10.1145/3397536.3428350], $C$ contextualizes sample confidence, as an identical ratio carries higher evidential certainty across dense vehicle queues than across sparse traffic. Because vehicles with fewer than $n$ tracking frames are excluded, $C$ conservatively undercounts traffic during rapid turnover, successfully protecting $R(t)$ from premature stationary misclassifications.

To capture the continuous intensity of surrounding vehicle movement that binary thresholding discards, the Mean Surrounding Displacement $\bar{d}$ is computed in metric units [Detection of Illegal Parking Events Using Spatial-Temporal Features https://doi.org/10.1145/3397536.3428350], [On kinematic waves. II. A theory of traffic flow on long crowded roads https://doi.org/10.1098/rspa.1955.0089]:

$$\bar{d} = \frac{1}{|\mathcal{S}(t_{\text{trig}})|} \sum_{v_i \in \mathcal{S}(t_{\text{trig}})} d(v_i, t_{\text{trig}})$$

Complementing the mean, the Surrounding Displacement Standard Deviation $\sigma_d$ measures the kinematic uniformity of surrounding traffic behavior [Detection of Illegal Parking Events Using Spatial-Temporal Features https://doi.org/10.1145/3397536.3428350], [Abnormal Vehicle Event Detection Based on Deep Learning https://doi.org/10.1109/cniot65435.2025.11070971]:

$$\sigma_d = \sqrt{\frac{1}{|\mathcal{S}(t_{\text{trig}})|} \sum_{v_i \in \mathcal{S}(t_{\text{trig}})} \left(d(v_i, t_{\text{trig}}) - \bar{d}\right)^2}$$

As demonstrated in traffic anomaly detection research [Abnormal Vehicle Event Detection Based on Deep Learning https://doi.org/10.1109/cniot65435.2025.11070971], low $\sigma_d$ indicates homogeneous flow (all vehicles flowing smoothly or all vehicles completely stopped in gridlock), whereas high $\sigma_d$ identifies heterogeneous, turbulent conditions (e.g., vehicles navigating around an obstruction).
...
Negative values indicate clearing congestion, whereas stable values indicate sustained conditions [Enhanced Detection of Illegally Parked Vehicles Using YOLO and Good Feature to Track Methods https://doi.org/10.1109/apsipaasc63619.2025.10849019], [Parking Time Violation Tracking Using YOLOv8 and Tracking Algorithms https://doi.org/10.3390/s23135843]. If all surrounding vehicles depart during monitoring such that $|\mathcal{S}(t_{\text{end}})| = 0$, $\Delta R$ is assigned a null representation, handled natively by XGBoost's sparsity-aware split finding algorithm [XGBoost: A Scalable Tree Boosting System https://doi.org/10.1145/2939672.2939785].

Finally, the Candidate Post-Trigger Displacement $d_{\text{cand}}$ verifies the candidate vehicle's individual dwell status [Enhanced Detection of Illegally Parked Vehicles Using YOLO and Good Feature to Track Methods https://doi.org/10.1109/apsipaasc63619.2025.10849019], [Parking Time Violation Tracking Using YOLOv8 and Tracking Algorithms https://doi.org/10.3390/s23135843]:
...
Regarding feature completeness, the vector intentionally omits static spatial attributes (e.g., distance to curb or lane markings), adhering strictly to lightweight edge-tracking principles without requiring computationally expensive lane-segmentation networks [Opportunities, Applications, and Challenges of Edge-AI Enabled Video Analytics in Smart Cities: A Systematic Review https://doi.org/10.1109/ACCESS.2023.3300658], [Parking Time Violation Tracking Using YOLOv8 and Tracking Algorithms https://doi.org/10.3390/s23135843]. Each feature represents an independent mathematical derivation: $R$ derives from binarized states, $\bar{d}$ from continuous displacements, $\sigma_d$ from second-order dispersion, $\Delta R$ from temporal progression, and $d_{\text{cand}}$ from candidate-specific motion. This formulation completely avoids linear dependencies and multicollinearity [XGBoost: A Scalable Tree Boosting System https://doi.org/10.1145/2939672.2939785]. In the isolated edge case where $|\mathcal{S}(t_{\text{trig}})| = 0$, the classifier is bypassed and the event defaults to standard temporal threshold logic, which is logged separately from model evaluation.
...
| Temporal trigger threshold     | $T$                   | 60 s                   | Literature consensus [Enhanced Detection of Illegally Parked Vehicles Using YOLO and Good Feature to Track Methods https://doi.org/10.1109/apsipaasc63619.2025.10849019], [Parking Violation Detection Using Computer Vision for Urban Traffic Management https://doi.org/10.1109/iciss63372.2025.11076537], [Outdoor Illegal Parking Detection System Using Convolutional Neural Network on Raspberry Pi https://doi.org/10.14419/ijet.v7i3.7.16197], [Illegal Parking Detection Using Gaussian Mixture Model and Kalman Filter https://doi.org/10.1109/aiccsa.2017.212]                |
...
where $p_o$ is the observed agreement and $p_e$ is the expected agreement by chance. A minimum $\kappa \geq 0.80$ is targeted, corresponding to the upper boundary of "Substantial" agreement (0.61 to 0.80) and the threshold for "Almost Perfect" agreement (0.81 to 1.00) [The Measurement of Observer Agreement for Categorical Data https://doi.org/10.2307/2529310]. If agreement falls below this threshold, disagreements will be adjudicated and the operational definition refined before re-annotation.
