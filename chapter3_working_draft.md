...
The recording locations comprise two urban roadway corridors in Quezon City, Metro Manila: P. A. Bernardo Street in Barangay Kaunlaran, Cubao (specifically the commercial corridor in the vicinity of 40 P. A. Bernardo Street) and Atherton Street in Barangay North Fairview (in the vicinity of the North Fairview Park Subdivision corridor). The selected sites satisfy four primary criteria to ensure the structural validity of the dataset. First, each site encompasses a legally designated restricted or no-parking zone, established either by posted signage or by statutory restriction under Section 46 of Republic Act No. 4136 (Land Transportation and Traffic Code of the Philippines), which prohibits parking within intersections, on crosswalks, within six meters of an intersection curb line, in front of driveways, or in any manner that obstructs moving traffic lanes. Second, the immediate recording locations are unsignalized, lacking physical traffic signals and forcing the proposed system to infer traffic states purely from the collective kinematic behavior of surrounding vehicles rather than external signal-state feeds or line-of-sight to signal heads. Third, both roadway corridors exhibit consistent traffic volume during weekday peak recording windows (16:00 to 18:00), ensuring that multiple surrounding vehicles are continuously present within the camera frame for contextual feature extraction. Finally, each site provides accessible, elevated physical vantage points (such as adjacent multi-story commercial balconies, elevated pedestrian footbridges, or secure roadside structures) permitting an elevated camera perspective to mitigate inter-vehicle occlusion.
...
In monocular surveillance geometry, the bottom-center point approximates the vehicle's physical contact point with the pavement, satisfying the coplanar road-surface constraint ($Z = 0$) required for planar homography projection. Utilizing the vertical centroid of the bounding box would introduce perspective height distortion that scales unfavorably with camera depression angle. Partial vertical occlusion from guardrails, roadside barriers, or intervening tall vehicles truncates the lower boundary of the bounding box, elevating the observed bottom-center coordinate above the true road surface ($Z > 0$). Under planar homography, projecting an elevated point produces an outward perspective distortion along the camera ray, projecting the estimated point further than the actual ground position; therefore, accurate metric displacement computation requires unobstructed road-contact visibility.
...
The physical formulation of $R(t)$ is rooted in Lighthill and Whitham's Kinematic Wave Theory [On kinematic waves. II. A theory of traffic flow on long crowded roads https://doi.org/10.1098/rspa.1955.0089]. In traffic flow theory, vehicles traversing an urban corridor are treated as interacting elements in a continuous flow where legitimate traffic stoppages represent collective, causally coupled deceleration shock waves propagating across upstream vehicles, driving $R(t) \to 1.0$. Conversely, an illegal parking event represents an isolated, individual anomaly where the candidate remains stationary while the surrounding traffic stream maintains independent movement, yielding $R(t) \to 0.0$.

To provide statistical weighting for the Stationary Ratio, the system records the Surrounding Vehicle Count $C = |\mathcal{S}(t_{\text{trig}})|$. In traffic flow dynamics, macroscopic stream properties depend on local vehicular concentration [Traffic Flow Dynamics: Data, Models and Simulation https://doi.org/10.1007/978-3-642-32460-4]. In this framework, $C$ serves as an empirical measure of local traffic density, establishing sample confidence: an identical stationary ratio carries higher statistical certainty across a dense platoon of vehicles than across sparse, intermittent traffic. Because vehicles with fewer than $n$ tracking frames are excluded, $C$ conservatively reflects established tracks, protecting $R(t)$ from premature stationary misclassifications during rapid traffic turnover.

To capture the continuous kinematic intensity of surrounding traffic that binary thresholding discards, the Mean Surrounding Displacement $\bar{d}$ is computed in metric units:

$$\bar{d} = \frac{1}{|\mathcal{S}(t_{\text{trig}})|} \sum_{v_i \in \mathcal{S}(t_{\text{trig}})} d(v_i, t_{\text{trig}})$$

This formulation reflects Wardrop's classical principle of space-mean traffic velocity [Some theoretical aspects of road traffic research https://doi.org/10.1680/ipeds.1952.11259], which measures the spatial displacement of traffic over an observed road section [Traffic Flow Dynamics: Data, Models and Simulation https://doi.org/10.1007/978-3-642-32460-4]. By quantifying average stream displacement across active surrounding vehicles, $\bar{d}$ provides a continuous measure of corridor mobility.

Complementing the mean, the Surrounding Displacement Standard Deviation $\sigma_d$ quantifies the kinematic dispersion of surrounding vehicle motion:

$$\sigma_d = \sqrt{\frac{1}{|\mathcal{S}(t_{\text{trig}})|} \sum_{v_i \in \mathcal{S}(t_{\text{trig}})} \left(d(v_i, t_{\text{trig}}) - \bar{d}\right)^2}$$

Following traffic flow dynamics and Wardrop's formulation of speed variance [Some theoretical aspects of road traffic research https://doi.org/10.1680/ipeds.1952.11259], velocity dispersion serves as a physical indicator of flow stability and turbulence [Traffic Flow Dynamics: Data, Models and Simulation https://doi.org/10.1007/978-3-642-32460-4]. Low $\sigma_d$ indicates laminar, homogeneous traffic conditions (where surrounding vehicles are either flowing uniformly or collectively halted in queue), whereas elevated $\sigma_d$ characterizes turbulent, heterogeneous conditions where individual vehicles decelerate, maneuver, or queue around an obstruction.

In the post-monitoring phase, the system evaluates the temporal evolution of the traffic stream through the Ratio Change $\Delta R$ across the monitoring window $W = 15$ seconds:

$$\Delta R = R(t_{\text{trig}} + W \times \text{fps}) - R(t_{\text{trig}})$$

This parameter operationalizes the hydrodynamic principles of shock wave dissipation and queue discharge established by Lighthill and Whitham [On kinematic waves. II. A theory of traffic flow on long crowded roads https://doi.org/10.1098/rspa.1955.0089]. When a temporary stoppage resolves, an acceleration expansion wave disperses the queue, producing a sharp decrease in the stationary proportion ($\Delta R < 0$). Conversely, sustained blockages or unyielding congestion exhibit stable stationary states ($\Delta R \approx 0$). If all surrounding vehicles depart during monitoring such that $|\mathcal{S}(t_{\text{end}})| = 0$, $\Delta R$ is assigned a null representation, handled natively by XGBoost's sparsity-aware split finding algorithm [XGBoost: A Scalable Tree Boosting System https://doi.org/10.1145/2939672.2939785].

Finally, the Candidate Post-Trigger Displacement $d_{\text{cand}}$ evaluates the candidate vehicle's individual microscopic trajectory continuity:
